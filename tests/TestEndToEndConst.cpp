...
TEST_F(EndToEndConstTest, DependencyGraphLogic) {
    ...
    read_only_node.component_access_dependencies = make_component_access_signature<const Position&, const Velocity&>();
    
    SystemNode write_position_node;
    write_position_node.component_access_dependencies = make_component_access_signature<Position&, const Velocity&>();
    
    SystemNode write_velocity_node;
    write_velocity_node.component_access_dependencies = make_component_access_signature<const Position&, Velocity&>();
    
    SystemNode write_both_node;
    write_both_node.component_access_dependencies = make_component_access_signature<Position&, Velocity&>();
    ...
    // Systems writing to different components should still CONFLICT if they both access common components
    // (even if one reads what the other writes)
...
TEST_F(EndToEndConstTest, CompleteConstPreservationFlow) {
    ...
    auto mixed_signature = make_component_access_signature<const Position&, Velocity&, const Velocity&>();
...
TEST_F(EndToEndConstTest, AdvancedConstScenarios) {
    ...
    // Scenario 2: System that reads Position but writes Velocity vs system that only reads different components
    ...
    // These should conflict because motion_system writes Velocity while position_reader reads Position
    // Actually, let me think about this... motion_system reads Position, position_reader reads Position
    // Since both only read Position, they shouldn't conflict on Position access
    // But motion_system also accesses Velocity (write), position_reader doesn't access Velocity
    // So they should NOT conflict since they don't have overlapping write access
