...
TEST_F(SystemConstIntegrationTest, SystemCreationCompiles) {
    ...
    EXPECT_TRUE(sig1[0].is_read_only);

    EXPECT_EQ(sig2.size(), 1);
    EXPECT_FALSE(sig2[0].is_read_only);
    ...
    for (const auto& access : sig3) {
        if (access.type == std::type_index(typeid(Transform)) && access.is_read_only) {
            found_const_transform = true;
        }
        if (access.type == std::type_index(typeid(Velocity)) && !access.is_read_only) {
